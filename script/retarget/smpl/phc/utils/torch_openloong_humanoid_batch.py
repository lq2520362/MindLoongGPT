...
import script.retarget.smpl.phc.utils.rotation_conversions as tRot

OPENLOONG_ROTATION_AXIS = torch.tensor([[
    [0, 0, 1], # head_yaw
    [0, -1, 0], # head_pitch
    
    [0, -1, 0], # r_shoulder_pitch
    [-1, 0, 0], # r_shoulder_roll
    [0, -1, 0], # r_shoulder_yaw
    [-1, 0, 0], # r_elbow
    [0, -1, 0], # r_wrist_pitch
    [1, 0, 0], # r_wrist_roll
    [0, 0, 1], # r_wrist_yaw
...
class Humanoid_Batch:
    ...
    def __init__(self, mjcf_file = f"robots/openloong/openloong.xml", device = torch.device("cpu")):
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        #     pcd.points = o3d.utility.Vector3dVector(wbody_pos[0][0])
        #     o3d.visualization.draw_geometries([pcd])
        
        
        wbody_rot = tRot.matrix_to_quaternion(wbody_mat)
        # wbody_pos:1*Q_LEN*23*3    wbody_rot:1*Q_LEN*23*4
        # if self.extend_hand:
        if return_full:
            # velocity
