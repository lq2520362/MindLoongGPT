...
from script.retarget.smpl.phc.utils.motion_lib_openloong import MotionLibLoong
...
openloongxml = "robot/openloong/OpenLoong.xml"
openloongurdf = "OpenLoong.urdf"
asset_descriptors = [
    AssetDesc(openloongurdf, False),
]
sk_tree = SkeletonTree.from_mjcf(openloongxml)

motion_file = "datas/retargeted/openloong/demo.pkl"

if os.path.exists(motion_file):
    print(f"loading {motion_file}")
else:
    raise ValueError(f"Motion file {motion_file} does not exist! Please run grad_fit_openloong.py first.")
...
motion_lib = MotionLibLoong(motion_file=motion_file, device=device, masterfoot_conifg=None, fix_height=False, multi_thread=False, mjcf_file=openloongxml)
num_motions = 1
curr_start = 0
...
while not gym.query_viewer_has_closed(viewer):
    ...
    ###########################################################################
    root_rot = torch.roll(root_rot, shifts=-1, dims=1)
    ...
    # data = torch.cat([root_pos,root_rot,dof_pos], dim=-1)[0].cpu().numpy()
    # datas.append(data)
    # print(dof_pos)
    for index in [1,2,3,4,5,6,14,16,21,22,23,27,28,30]:
        dof_pos[0][index] *= -1
    # print(dof_pos)
